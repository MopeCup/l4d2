...
bool  g_bReturnType, g_bLeftSafeArea;
...
void HealClient(int client)
{
	...
	if (IsValidSur(client) && IsPlayerAlive(client) && (L4D_IsInFirstCheckpoint(client) || L4D_IsInLastCheckpoint(client)))
	{
		...
		if (allHealth < safeNaps)
		{
			...
			{
				SetEntProp(client, Prop_Data, "m_iHealth", RoundToCeil(safeNaps));
				SetEntPropFloat(client, Prop_Send, "m_healthBuffer", allHealth - safeNaps);
			}
			else {
				//恢复原血量
				SetEntProp(client, Prop_Data, "m_iHealth", realHealth);
...
{
	if (GetEntProp(jockey, Prop_Send, "m_zombieClass") != 5 || GetEntPropEnt(jockey, Prop_Send, "m_hGroundEntity") > -1 || GetEntityMoveType(jockey) != MOVETYPE_WALK || GetEntProp(jockey, Prop_Send, "m_nWaterLevel") >= 3 || GetEntPropEnt(jockey, Prop_Send, "m_jockeyVictim") > -1)
		return false;
