...
Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
    ...
    if (attacker == victim || !IsClientAndInGame(attacker) || !IsClientAndInGame(victim) || !IsValidEntity(inflictor) || ShouldPluginStop(victim) || GetClientTeam(attacker) == L4D_TEAM_INFECTED || GetClientTeam(victim) != L4D_TEAM_SURVIVOR) return Plugin_Continue;
...
void Event_Hurt(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if (g_bEnable == false || attacker == victim || !IsClientAndInGame(attacker) || !IsClientAndInGame(victim) || ShouldPluginStop(victim) || GetClientTeam(victim) != L4D_TEAM_SURVIVOR || GetClientTeam(attacker) == L4D_TEAM_INFECTED || !IsPlayerAlive(victim) || damage <= 0 || damage <= g_iDamageShield) { return; }
...
void Event_IncapacitatedStart(Event event, const char[] name, bool dontBroadcast)
{
    ...
    int victim   = GetClientOfUserId(event.GetInt("userid"));
    int attacker = GetClientOfUserId(event.GetInt("attacker"));
    if (g_bEnable == false || attacker == victim || !IsClientAndInGame(attacker) || !IsClientAndInGame(victim) || ShouldPluginStop(victim) || GetClientTeam(attacker) == L4D_TEAM_INFECTED || GetClientTeam(victim) != L4D_TEAM_SURVIVOR) { return; }
...
bool ShouldDisableFF(int attacker, int victim)
{
    ...
    if (g_fCVDistance > 0.0)
    {
        ...
        if (fDistance <= g_fCVDistance || IsSINearby(fPos_2))
            return true;
    }
...
bool IsSINearby(const float fPos[3])
{
    for (int i = 1; i <= MaxClients; i++)
    {
        if (!IsClientInGame(i) || GetClientTeam(i) != 3)
            continue;
        float fPos_SI[3];
        GetClientAbsOrigin(i, fPos_SI);
        if (GetVectorDistance(fPos_SI, fPos) < 100.0)
            return true;
    }
    return false;
}