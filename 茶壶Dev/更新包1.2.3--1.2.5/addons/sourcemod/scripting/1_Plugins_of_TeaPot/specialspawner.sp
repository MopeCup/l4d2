...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max) {
	CreateNative("SS_GetSILimit", NativeGetSILimit);
	CreateNative("SS_GetSISpawnTime", NativeGetSISpawnTime);
	//CreateNative("GetSISpawnDis", NativeGetSISpawnDis);
	//CreateNative("SetTankFightSILimt", NativeSetTankFightSILimt);
...
public void OnPluginStart() {
	...
	g_cSpawnWeights[SI_BOOMER] =	CreateConVar("ss_boomer_weight",		"50",						"boomer产生比重", _, true, 0.0);
	g_cSpawnWeights[SI_HUNTER] =	CreateConVar("ss_hunter_weight",		"100",						"hunter产生比重", _, true, 0.0);
	g_cSpawnWeights[SI_SPITTER] =	CreateConVar("ss_spitter_weight",		"50",						"spitter产生比重", _, true, 0.0);
	...
	g_cTankStatusWeights =			CreateConVar("ss_tankstatus_weights",	"100;50;100;50;100;100",	"坦克产生后每种特感比重的自定义参数");
	...
	g_cSpawnRangeMin =				CreateConVar("ss_spawnrange_min",		"300.0",					"特感最小生成距离", _, true, 0.0);
	g_cSpawnRangeMax =				CreateConVar("ss_spawnrange_max",		"1500.0",					"特感最大生成距离", _, true, 0.0);

	g_cFirstSpawnTime = 			CreateConVar("ss_first_time",			"10.0",						"玩家离开安全区域后第一波特感的刷新时间", _, true, 0.0);
	g_cSpawnForm = 					CreateConVar("ss_spawn_form", 			"0", 						"遵从什么刷怪规则[0: 轮换刷特, 1: 概率刷特]", _, true, 0.0, true, 1.0);
...
//-----Native Begin-----
int NativeGetSILimit(Handle plugin, int numParams){
	int sILimit = g_cSILimit.IntValue;
	int spawnSize = g_cSpawnSize.IntValue;

	int baseLimit = g_cBaseLimit.IntValue;
	int extraLimit = g_cExtraLimit.IntValue;
	int baseSize = g_cBaseSize.IntValue;
	int extarSize = g_cExtraSize.IntValue;
	...
	int finale_SILimit;
	if(surNum > 0){
		int num1 = baseLimit + extraLimit * surNum;
		int num2 = baseSize + extarSize * surNum;
		finale_SILimit = num1 < num2 ? num1 : num2;
	}
	else{
		finale_SILimit = sILimit < spawnSize ? sILimit : spawnSize;
	}
	return finale_SILimit;
}

int NativeGetSISpawnTime(Handle plugin, int numParams){
	float fSpawnTimeMax = GetConVarFloat(g_cSpawnTimeMax);
	return RoundToNearest(fSpawnTimeMax);
}

// int NativeGetSISpawnDis(Handle plugin, int numParams){
// 	int iTankNum = GetNativeCell(1);
// 	if(iTankNum > 0){
