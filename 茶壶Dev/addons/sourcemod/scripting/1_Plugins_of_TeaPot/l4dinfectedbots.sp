...
public void OnPluginStart()
{
	...
	g_hSpawnLimits[SI_BOOMER] = 		CreateConVar("l4d_infectedbots_boomer_limit", 							"0", 		"Sets the limit for boomers spawned by the plugin", FCVAR_NOTIFY, true, 0.0);
	g_hSpawnLimits[SI_SMOKER] = 		CreateConVar("l4d_infectedbots_smoker_limit", 							"1", 		"Sets the limit for smokers spawned by the plugin", FCVAR_NOTIFY, true, 0.0);
	...
	if (g_bL4D2Version)
	{
		g_hSpawnLimits[SI_SPITTER] = 	CreateConVar("l4d_infectedbots_spitter_limit", 							"0", 		"Sets the limit for spitters spawned by the plugin", FCVAR_NOTIFY, true, 0.0);
	...
	h_WitchLimit = 						CreateConVar("l4d_infectedbots_witch_max_limit", 						"0", 		"Sets the limit for witches spawned by the plugin (does not affect director witches)", FCVAR_NOTIFY, true, 0.0);
	
	h_TankLimit = 						CreateConVar("l4d_infectedbots_tank_limit", 							"0", 		"Sets the limit for tanks spawned by the plugin (does not affect director tanks)", FCVAR_NOTIFY, true, 0.0);
	h_PlayerAddTankLimitScale = 		CreateConVar("l4d_infectedbots_add_tanklimit_scale",					"0", 		"If server has more than 4+ alive players, how many tanks on the field = 'tank_limit' + [(alive players - 4) ÷ 'add_tanklimit_scale' × 'add_tanklimit'].", FCVAR_NOTIFY, true, 1.0);
	h_PlayerAddTankLimit = 				CreateConVar("l4d_infectedbots_add_tanklimit", 							"0", 		"If server has more than 4+ alive players, increase the certain value to 'l4d_infectedbots_tank_limit' each 'l4d_infectedbots_add_tanklimit_scale' players joins", FCVAR_NOTIFY, true, 0.0);
	h_TankSpawnFinal = 					CreateConVar("l4d_infectedbots_tank_spawn_final", 						"0", 		"If 1, still spawn tank in final stage rescue (does not affect director tanks)", FCVAR_NOTIFY, true, 0.0, true, 1.0);

	h_MaxPlayerZombies = 				CreateConVar("l4d_infectedbots_max_specials", 							"6", 		"Defines how many special infected can be on the map on all gamemodes(does not count witch on all gamemodes, count tank in all gamemode)", FCVAR_NOTIFY, true, 0.0);
	h_PlayerAddZombiesScale = 			CreateConVar("l4d_infectedbots_add_specials_scale", 					"0", 		"If server has more than 4+ alive players, how many special infected = 'max_specials' + [(alive players - 4) ÷ 'add_specials_scale' × 'add_specials'].", FCVAR_NOTIFY, true, 1.0);
	h_PlayerAddZombies = 				CreateConVar("l4d_infectedbots_add_specials", 							"0", 		"If server has more than 4+ alive players, increase the certain value to 'l4d_infectedbots_max_specials' each 'l4d_infectedbots_add_specials_scale' players joins", FCVAR_NOTIFY, true, 0.0);

	h_TankHealthAdjust = 				CreateConVar("l4d_infectedbots_adjust_tankhealth_enable", 				"0", 		"If 1, adjust and overrides tank health by this plugin.", FCVAR_NOTIFY, true, 0.0,true, 1.0);
	...
	h_PlayerAddTankHealthScale = 		CreateConVar("l4d_infectedbots_add_tankhealth_scale", 					"0", 		"If server has more than 4+ alive players, how many Tank Health = 'default_tankhealth' + [(alive players - 4) ÷ 'add_tankhealth_scale' × 'add_tankhealth'].", FCVAR_NOTIFY, true, 1.0);
	h_PlayerAddTankHealth = 			CreateConVar("l4d_infectedbots_add_tankhealth", 						"0", 		"If server has more than 4+ alive players, increase the certain value to 'l4d_infectedbots_default_tankhealth' each 'l4d_infectedbots_add_tankhealth_scale' players joins", FCVAR_NOTIFY, true, 0.0);
	h_InfectedSpawnTimeMax = 			CreateConVar("l4d_infectedbots_spawn_time_max", 						"5", 		"Sets the max spawn time for special infected spawned by the plugin in seconds.", FCVAR_NOTIFY, true, 1.0);
	h_InfectedSpawnTimeMin = 			CreateConVar("l4d_infectedbots_spawn_time_min", 						"5", 		"Sets the minimum spawn time for special infected spawned by the plugin in seconds.", FCVAR_NOTIFY, true, 1.0);
	...
	h_JoinableTeamsAnnounce = 			CreateConVar("l4d_infectedbots_coop_versus_announce", 					"0", 		"If 1, clients will be announced to on how to join the infected team", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	h_HumanCoopLimit = 					CreateConVar("l4d_infectedbots_coop_versus_human_limit", 				"0", 		"Sets the limit for the amount of humans that can join the infected team in coop/survival/realism.", FCVAR_NOTIFY, true, 0.0);
	...
	h_AdjustSpawnTimes = 				CreateConVar("l4d_infectedbots_adjust_spawn_times", 					"0", 		"If 1, The plugin will adjust spawn timers depending on the gamemode and human players on infected team", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	h_ReducedSpawnTimesOnPlayer = 		CreateConVar("l4d_infectedbots_adjust_reduced_spawn_times_on_player", 	"0", 		"Reduce certain value to maximum spawn timer based per alive player", FCVAR_NOTIFY, true, 0.0);
	...
	h_SpawnDistanceMin = 				CreateConVar("l4d_infectedbots_spawn_range_min", 						"550", 		"The minimum of spawn range for infected. (default: 550, coop/realism only)\nThis cvar will also affect common zombie spawn range and ghost infected player spawn range", FCVAR_NOTIFY, true, 0.0, true, 550.0);
	...
	h_SpawnTankProbability = 			CreateConVar("l4d_infectedbots_tank_spawn_probability", 				"0", 		"When each time spawn S.I., how much percent of chance to spawn tank", FCVAR_NOTIFY, true, 0.0, true, 100.0);
	h_ZSDisableGamemode = 				CreateConVar("l4d_infectedbots_sm_zs_disable_gamemode", 				"0", 		"Disable sm_zs in these gamemode (0: None, 1: coop/realism, 2: versus/scavenge, 4: survival, add numbers together)", FCVAR_NOTIFY, true, 0.0, true, 7.0);
	h_CommonLimitAdjust = 				CreateConVar("l4d_infectedbots_adjust_commonlimit_enable", 				"0", 		"If 1, adjust and overrides zombie common limit by this plugin.", FCVAR_NOTIFY, true, 0.0,true, 1.0);
	...
	h_InfectedSpawnSameFrame = 			CreateConVar("l4d_infectedbots_spawn_on_same_frame", 					"1", 		"If 1, infected bots can spawn on the same game frame (careful, this could cause sever laggy)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	h_WhereToSpawnInfected = 			CreateConVar("l4d_infectedbots_spawn_where_method", 					"1", 		"Where to spawn infected? 0=Near the first ahead survivor. 1=Near the random survivor", FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
void ConVarMaxPlayerZombies(ConVar convar, const char[] oldValue, const char[] newValue)
{
	g_iMaxPlayerZombies = h_MaxPlayerZombies.IntValue;
...
void ConVarDistanceChanged(ConVar convar, const char[] oldValue, const char[] newValue)
{
	GetSpawnDisConvars();
}

void ConVarCoopVersus(ConVar convar, const char[] oldValue, const char[] newValue)
{
...
Action JoinInfected(int client, int args)
{
	if( g_bCvarAllow == false) return Plugin_Handled;
...
Action JoinSurvivors(int client, int args)
{
	if( g_bCvarAllow == false) return Plugin_Handled;
...
Action ZombieClassTimer(Handle timer, int client)
{
	if (client)
	{
...
Action Timer_Spawn_InfectedBot(Handle timer, int index)
{
	...
	else
	{
		...
		if (bSpawnSuccessful && IsValidClient(bot))
		{
			...
			if(!g_bIsCoordination) CreateTimer(0.05, CheckIfBotsNeededLater, 1, TIMER_FLAG_NO_MAPCHANGE);
		}
		else
		{
			if(!g_bIsCoordination) CreateTimer(0.1, CheckIfBotsNeededLater, 0, TIMER_FLAG_NO_MAPCHANGE);
		}
	}
...
void cvarZombieHPChanged(ConVar convar, const char[] oldValue, const char[] newValue)
{
...
Action Timer_CheckSpawn(Handle timer)
{
	...
	PrintToChatAll("BotAlive: %d, RealAlive: %d, RealDeath: %d, spawntimers: %d, g_iMaxPlayerZombies: %d", 
		iInfectedBotAliveCount, iInfectedRealAliveCount, iInfectedRealDeathQueue, count, g_iMaxPlayerZombies);
	*/
	...
	if (L4D_HasPlayerControlledZombies())
	{
		...
		for(int index = 1; index <= L4D_MAXPLAYERS; index++)
		{
			...
			{
				spawntimers++;
				if(iInfectedBotAliveCount + iInfectedRealAliveCount + iInfectedRealDeathQueue + spawntimers > g_iMaxPlayerZombies) 
				{
	...
	else
	{
		...
		for(int index = 1; index <= L4D_MAXPLAYERS; index++)
		{
			...
			{
				spawntimers++;
				if(iInfectedBotAliveCount + iInfectedRealAliveCount + spawntimers > g_iMaxPlayerZombies) 
				{
					//PrintToChatAll("----delete----");
					if(InfectedBotQueue > 0) InfectedBotQueue--;
					delete SpawnInfectedBotTimer[index];
...
void ConVarChanged_BalanceUpdate(ConVar convar, const char[] oldValue, const char[] newValue)
{
	g_bCommonLimitAdjust = h_CommonLimitAdjust.BoolValue;
	g_bTankHealthAdjust = h_TankHealthAdjust.BoolValue;
	TankHealthCheck();
...
void ConVarChanged_TankLimitUpdate(ConVar convar, const char[] oldValue, const char[] newValue)
{
	int iAliveSurplayers = CheckAliveSurvivorPlayers_InSV();
