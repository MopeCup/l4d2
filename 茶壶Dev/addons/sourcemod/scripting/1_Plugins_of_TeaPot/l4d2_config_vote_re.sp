...
{
	g_hConfigFile = CreateConVar("l4d2_config_vote_file", "data/l4d2_config_vote.cfg", "配置文件路径", FCVAR_NOTIFY | FCVAR_DONTRECORD);
	...
	g_hConfigFile.AddChangeHook(ResetPath);

	//RegConsoleCmd("sm_vote", Cmd_Vote);
	RegConsoleCmd("sm_vote", Cmd_Vote);
	RegConsoleCmd("sm_vt", Cmd_Vote);

	RegPluginLibrary("l4d2_config_vote");
}

public void ResetPath(ConVar convar, const char[] oldValue, const char[] newValue){
	SetPath();
}

void SetPath(){
	GetConVarString(g_hConfigFile, g_sConfigFile, sizeof(g_sConfigFile));

	BuildPath(Path_SM, g_sConfig, sizeof(g_sConfig), g_sConfigFile);
	if (!FileExists(g_sConfig))
		SetFailState("%s file does not exist!", g_sConfig);
}
