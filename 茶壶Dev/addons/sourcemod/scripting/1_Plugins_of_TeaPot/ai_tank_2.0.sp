...
{
	// 连跳相关
	g_hAllowBhop = CreateConVar("ai_TankAllowBhop", "0", "是否开启坦克连跳", CVAR_FLAG, true, 0.0, true, 1.0);
	...
	g_hConsumeInfSub = CreateConVar("ai_TankConsumeInfSub", "7", "当前特感少于等于特感上限减去这个值的时，坦克可以消耗", CVAR_FLAG, true, 0.0);
	...
	g_hConsumeHealth = CreateConVar("ai_TankConsumeHealth", "8000", "坦克血量少于这个值时强制压制", CVAR_FLAG, true, 0.0);
	g_hVomitAttackNum = CreateConVar("ai_TankVomitAttackNum", "1", "有这个值的生还者在坦克消耗时被喷，坦克会强制压制", CVAR_FLAG, true, 0.0);
	g_hConsumeIncap = CreateConVar("ai_TankConsumeIncapNum", "2", "坦克强制压制时，如果令这个数量的生还者倒地，如果可以消耗则继续消耗", CVAR_FLAG, true, 0.0);
