...
ConVar cmeleedefault, cinitialgundefault, csafearea, cAdvancedGunOpen, cAdminMenuOpen;
...
public void OnPluginStart()
{
	...
	cAdminMenuOpen		= CreateConVar("l4d2_AdminMenuOpen",		"0",	"默认是否开启管理员选项",		FCVAR_NOTIFY, true,	0.0, true, 1.0);
	...
	cAdminMenuOpen.AddChangeHook(IsOtherConVarChanged);
}
...
void IsGetOtherCvars()
{
	...
	bAdminMenuOpen 			= 	cAdminMenuOpen.BoolValue;
}
...
Action ServerInfo(Handle timer, int userid)
{
	...
	if (client && IsClientInGame(client) && GetClientTeam(client) == 2)
	{
		if((bsafearea && IsClientInSafeArea(client)) || !bsafearea)
			CPrintToChat(client, "{green}[提示]{blue}同时按住{green} E + R {blue}键可获取枪械和近战");
	}
	return Plugin_Stop;
}
...
void IsChooseItem(int client)
{
	...
	if (iGetClientImmunityLevel(client) > 49 && bAdminMenuOpen) 
			...
			if((bsafearea && IsClientInSafeArea(client)) || !bsafearea)
			{
				...
				if (IsValidEntity(weaponid))
				{
					...
					if(StrContains(classname, g_sWeaponName[0][param2], true) == -1 || StrContains(classname, "pistol", true) !=-1)
					{
						...
						CPrintToChatAll( "{blue}❀ {green}%N {blue}获取了 {olive}%s", client, g_sItemName[0][param2]);
					}
					else
						return 0;
				}
				...
				CPrintToChat(client, "{green}[提示] {blue}请在安全区域内使用 {green}E + R {blue}功能");
		}
			...
			if((bsafearea && IsClientInSafeArea(client)) || !bsafearea)
			{
				...
				if (IsValidEntity(weaponid))
				{
					...
					else
					{
						...
						vCheatCommand(client, line);
						CPrintToChatAll( "{blue}❀ {green}%N {blue}获取了 {olive}%s", client, g_sItemName[1][param2]);
					}
				}
				else
				{
			...
			if((bsafearea && IsClientInSafeArea(client)) || !bsafearea)
			{
				...
				if (IsValidEntity(weaponid))
				{
					...
					else
					{
						...
						vCheatCommand(client, line);
						CPrintToChatAll( "{blue}❀ {green}%N {blue}获取了 {olive}%s", client, g_sItemName[2][param2]);
					}
				}
				else
				{
	...
	switch (action) 
	{
		case MenuAction_Select: 
		{
			if((bsafearea && IsClientInSafeArea(client)) || !bsafearea)
			{
				...
				vCheatCommand(client, line);
				CPrintToChatAll( "{blue}❀ {green}%N {blue}获取了 {olive}%s", client, g_sItemName[3][num]);
			}
			else
				CPrintToChat(client, "{green}[提示] {blue}请在安全区域内使用 {green}E + R {blue}功能");
		}
			...
			switch(sItem[0])
				{
					case 'e':
					{
						...
						CPrintToChatAll( bmeleedefault ? "{blue}管理员 {green}%N {blue}开启了副武器菜单" : "{blue}管理员 {green}%N {blue}关闭了副武器菜单", client);
					}
					case 'f':
					{
						binitialgundefault = !binitialgundefault;
						CPrintToChatAll( binitialgundefault ? "{blue}管理员 {green}%N {blue}开启了小枪菜单" : "{blue}管理员 {green}%N {blue}关闭了小枪菜单", client);
					}
					case 'g':
					{
						MenuAdvancedGunOpen = !MenuAdvancedGunOpen;
						CPrintToChatAll( MenuAdvancedGunOpen ? "{blue}管理员 {green}%N {blue}开启了大枪菜单" : "{blue}管理员 {green}%N {blue}关闭了大枪菜单", client);
					}
					case 'h':
					{
						MenuItemAllow(client,0);
						// CPrintToChatAll( ItemCount ? "{blue}管理员 {green}%N {blue}开启了物品菜单" : "{blue}管理员 {green}%N {blue}关闭了物品菜单", client);
					}
					case 'i':
					{
						bsafearea = !bsafearea;
						CPrintToChatAll( bsafearea ? "{blue}管理员 {green}%N {blue}}已禁止安全区域外使用 {green}E + R {blue}功能" : "{blue}管理员 {green}%N {green}已允许安全区域外使用 {green}E + R {blue}功能", client);
					}
				}
			}
		}
